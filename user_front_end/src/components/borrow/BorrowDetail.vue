        ...
        <div class="col-lg-9">
          ...
          <div v-else>
            ...
            <div class="row g-5">
              ...
              <div class="col-md-4">
                <router-link
                  :to="{ name: 'book.detail', params: { id: borrow.sach._id } }"
                >
...
const router = useRouter(); // thêm nếu cần
...
const cancel = async () => {
  ...
  if (isConfirmed) {
    try {
      await BorrowService.cancel(route.params.id);
      await loadBorrowDetail();
      Swal.fire("Đã từ chối", "Phiếu mượn đã bị hủy", "success");
    } catch (err) {
      Swal.fire("Lỗi", err.message || "Không thể từ chối", "error");
    }
  }
};
